...
| 图片 OCR 录入 | 条件式实现 | 前端可裁剪和校正；识别依赖 PaddleOCR、本地模型或外部 OCR/Kimi 服务 |
...
- 摄取：PyMuPDF、MinerU、Kimi Vision、PaddleOCR 或外部 OCR
- 测试：pytest、Vitest
...
- Node.js 与 npm。
- 可选：DeepSeek/OpenAI 兼容 LLM、Moonshot/Kimi、MinerU、OCR 服务。
...
| <code>OCR_API_URL</code> | 可选 OCR 服务地址 | 外部 OCR 时 |
...
- MinerU、OCR 和 Kimi Vision 不随仓库作为完整服务交付。
